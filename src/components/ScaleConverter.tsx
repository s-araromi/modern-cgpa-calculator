...
export interface ScaleConverterProps {
  scale?: GradeScale;
  onScaleChange?: (newScale: GradeScale) => void;
}

const ScaleConverter: React.FC<ScaleConverterProps> = ({ 
  scale: initialScale = '4.0', 
  onScaleChange 
}) => {
  const [fromScale, setFromScale] = useState<GradeScale>(initialScale);
  const [toScale, setToScale] = useState<GradeScale>('5.0');
  ...
  const handleFromScaleChange = (newScale: GradeScale) => {
    setFromScale(newScale);
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4 justify-center">
        <div className="space-y-2">
          ...
          <select
            value={fromScale}
            onChange={(e) => handleFromScaleChange(e.target.value as GradeScale)}
        ...
        <div className="space-y-2">
          ...
          <select
            value={toScale}
            onChange={(e) => setToScale(e.target.value as GradeScale)}
            className="w-full px-4 py-2 rounded-lg bg-white/50 border border-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500"
          >
            <option value="4.0">4.0</option>
            <option value="5.0">5.0</option>
